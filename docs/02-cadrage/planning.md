# Planning

!!! note "Trame à compléter"
    Les étapes ci-dessous reprennent les phases du projet ([Objectifs et livrables](index.md)), découpées en tâches plus précises. Les dates sont volontairement laissées vides : à renseigner en équipe avec le tuteur dès la réunion de lancement, à la rentrée de septembre.

## Jalons clés

Dates de passage d'une phase à l'autre / points de validation avec le tuteur.

| Jalon | Date prévue | Date réelle | Statut |
|---|---|---|---|
| Réunion de lancement du projet | | | ⬜ |
| Constitution de l'équipe et répartition des rôles | | | ⬜ |
| Validation de l'analyse préalable (cibles, audit, benchmark) | | | ⬜ |
| Validation de l'arborescence cible | | | ⬜ |
| Validation des maquettes / wireframes | | | ⬜ |
| Début de l'intégration Elementor | | | ⬜ |
| Recette finale | | | ⬜ |
| Mise en production | | | ⬜ |
| Soutenance / présentation finale | | | ⬜ |

!!! tip "Légende statut"
    ⬜ à venir · 🔄 en cours · ✅ terminé · ⚠️ en retard

## Détail des étapes par phase

### 1. Cadrage

| Étape | Début | Fin | Responsable | Statut |
|---|---|---|---|---|
| Réunion de lancement avec le tuteur | | | | ⬜ |
| Lecture du brief (ce site) par toute l'équipe | | | | ⬜ |
| Répartition des rôles ([Équipe](equipe.md)) | | | | ⬜ |
| Premier contact avec le commanditaire / référent du site actuel | | | | ⬜ |

### 2. Analyse préalable

Voir détail méthodologique : [Analyse préalable](../03-analyse-prealable/index.md)

| Étape | Début | Fin | Responsable | Statut |
|---|---|---|---|---|
| Audit du site existant (inventaire de contenu, technique, responsive) | | | | ⬜ |
| Recherche de cibles / personas | | | | ⬜ |
| Définition des objectifs du site | | | | ⬜ |
| Benchmark (sites comparables) | | | | ⬜ |
| Synthèse d'analyse et restitution au tuteur | | | | ⬜ |

### 3. Conception

| Étape | Début | Fin | Responsable | Statut |
|---|---|---|---|---|
| Arborescence cible du nouveau site | | | | ⬜ |
| Wireframes / maquettes des pages clés | | | | ⬜ |
| Choix éditoriaux (ton, structure des pages) | | | | ⬜ |
| Présentation et validation des maquettes | | | | ⬜ |

### 4. Réalisation (Elementor)

Voir détail technique : [Refonte](../04-refonte/index.md)

| Étape | Début | Fin | Responsable | Statut |
|---|---|---|---|---|
| Mise en place de la charte de styles (couleurs, typographies) | | | | ⬜ |
| Intégration de la page d'accueil | | | | ⬜ |
| Intégration des pages clés (formation, admissions, contact...) | | | | ⬜ |
| Intégration des contenus restants | | | | ⬜ |
| Vérification responsive au fil de l'intégration | | | | ⬜ |

### 5. Tests et recette

| Étape | Début | Fin | Responsable | Statut |
|---|---|---|---|---|
| Tests multi-appareils (mobile, tablette, desktop) | | | | ⬜ |
| Tests multi-navigateurs | | | | ⬜ |
| Relecture éditoriale (orthographe, cohérence) | | | | ⬜ |
| Corrections suite aux tests | | | | ⬜ |
| Validation finale par le tuteur / commanditaire | | | | ⬜ |

### 6. Livraison

| Étape | Début | Fin | Responsable | Statut |
|---|---|---|---|---|
| Mise en production du site refondu | | | | ⬜ |
| Rédaction de la documentation de passation | | | | ⬜ |
| Préparation de la soutenance | | | | ⬜ |
| Soutenance / présentation finale | | | | ⬜ |

!!! warning "Dépendances à anticiper"
    La phase **Réalisation** ne devrait démarrer qu'une fois l'arborescence cible et les maquettes validées (fin de phase **Conception**) — commencer l'intégration Elementor avant cette validation expose à devoir tout refaire. Idem, ne pas attendre la fin de l'intégration pour commencer les tests responsive : les vérifier en continu, page par page.
