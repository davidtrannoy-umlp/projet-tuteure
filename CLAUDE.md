...
6. **Historique de fiabilité du déploiement (juillet 2026)** : les deux méthodes de déploiement Pages ont montré une instabilité côté GitHub sur ce dépôt durant cette période :
   - Le pipeline **legacy** (`mkdocs gh-deploy` → branche `gh-pages` → build Jekyll) a échoué 4 fois de suite le 02/07 (`status: errored`, `duration: 0`, sans rapport avec le contenu).
   - Le pipeline **Actions natif** (`upload-pages-artifact` + `deploy-pages`, testé brièvement le 02-03/07) a lui aussi échoué (`deployment_queued` bloqué jusqu'au timeout, puis `Deployment failed, try again later`), y compris après résolution d'un incident GitHub Pages confirmé sur githubstatus.com (16:54→18:25 UTC le 02/07).
   - **Conclusion** : ces échecs semblaient liés à une instabilité ponctuelle de l'infra Pages de GitHub plutôt qu'à l'une ou l'autre méthode. On est **revenu au pipeline legacy** (`mkdocs gh-deploy`, Settings → Pages → Source = "Deploy from a branch" → `gh-pages` / `/`) par choix du tuteur, sans garantie que ce soit plus fiable que Actions dans l'absolu — si les échecs reprennent, vérifier d'abord https://www.githubstatus.com/ avant de modifier la config.
